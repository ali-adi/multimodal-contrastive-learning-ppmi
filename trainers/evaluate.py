...
from utils.utils import grab_arg_from_checkpoint, grab_hard_eval_image_augmentations, grab_wids
...
def load_datasets(hparams):
    if hparams.datatype == 'imaging' or hparams.datatype == 'multimodal':
        ...
        raise Exception('argument datatype must be set to imaging, tabular, multimodal, imaging_and_tabular, or ppmi')
...
def evaluate(hparams, logger):
    ...
    logdir = os.path.join(hparams.data_base, 'runs', 'eval', 'checkpoints')
    ...
    logger.log_metrics({f'best.val.{hparams.eval_metric}': model.best_val_score})
